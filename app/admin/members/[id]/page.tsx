...
type Donation = { id: string; amount: number; date: string; method?: string; receipt_no?: string; donation_month?: string; donation_end_month?: string; coverage_start_month?: string; coverage_end_month?: string; note?: string };
...
export default function AdminMemberDetailPage() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        setAllocations((allocs || []) as PaymentAllocation[]);
  ...
  const totalUnallocated = ledger.reduce((sum, row) => sum + row.unallocated, 0);
  ...
  return (
    <div className="p-4 sm:p-8 space-y-6">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-5 gap-3 sm:gap-5">{[{ label: "মোট প্রত্যাশিত", value: totalExpected, color: "text-gray-900" }, { label: "মোট বরাদ্দ জমা", value: totalPaid, color: "text-emerald-600" }, { label: "মোট বকেয়া", value: totalDue, color: "text-rose-600" }, { label: "অবণ্টিত / অতিরিক্ত", value: totalUnallocated, color: "text-amber-600" }, { label: "শেষ জমা", value: lastPayment ? Number(lastPayment.amount) : 0, color: "text-blue-600" }].map((card) => <div key={card.label} className="card-premium p-4 sm:p-6"><p className="text-xs text-gray-400 font-bold mb-2">{card.label}</p><p className={`text-xl sm:text-2xl font-bold ${card.color}`}>৳{card.value.toLocaleString("bn-BD")}</p>{card.label === "শেষ জমা" && lastPayment && <p className="text-[10px] text-gray-400 mt-1">{new Date(lastPayment.date).toLocaleDateString("bn-BD")}</p>}</div>)}</div>
      <div className="card-premium p-4 sm:p-8"><div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6"><div><h2 className="text-xl font-bold font-tiro text-gray-900">Payment History ও Member Ledger</h2><p className="text-sm text-gray-400 mt-1">মাসভিত্তিক জমা ও বকেয়ার হিসাব</p></div><select value={year} onChange={(e) => setYear(Number(e.target.value))} className="rounded-xl border border-gray-200 px-4 py-2 font-bold text-sm">{[year - 1, year, year + 1].map((item) => <option key={item} value={item}>{item}</option>)}</select></div><div className="overflow-x-auto"><table className="w-full min-w-[720px] text-left"><thead><tr className="border-b border-gray-100 text-xs text-gray-400"><th className="py-3">মাস</th><th>প্রত্যাশিত</th><th>জমা</th><th>অবণ্টিত</th><th>বাকি</th><th>স্ট্যাটাস</th><th>রসিদ</th></tr></thead><tbody>{ledger.map((row) => { const receipt = row.donations[0]; return <tr key={row.month} className="border-b border-gray-50 text-sm"><td className="py-4 font-bold text-gray-800"><Calendar size={14} className="inline mr-2 text-emerald-600" />{formatMonth(row.month)}</td><td>৳{row.expected.toLocaleString("bn-BD")}</td><td className="font-bold text-emerald-600">৳{row.paid.toLocaleString("bn-BD")}</td><td className="font-bold text-amber-600">৳{row.unallocated.toLocaleString("bn-BD")}</td><td className="font-bold text-rose-600">৳{row.remaining.toLocaleString("bn-BD")}</td><td><span className={`px-3 py-1 rounded-full text-xs font-bold ${statusClass[row.status]}`}>{statusLabel[row.status]}{row.unallocated > 0 ? " + অতিরিক্ত" : ""}</span></td><td>{receipt ? <a href={`/api/receipts/${receipt.id}`} target="_blank" rel="noreferrer" className="inline-flex items-center gap-1 text-emerald-600 font-bold text-xs"><ExternalLink size={13} /> দেখুন</a> : <span className="text-gray-300">—</span>}</td></tr>; })}</tbody></table></div></div>
