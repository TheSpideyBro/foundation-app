-- ============================================================
-- FIX-RLS-POLICIES-V4  —  fully idempotent; safe to run
-- multiple times in Supabase Dashboard > SQL Editor
--
-- Changes vs V3:
--   * Now drops EVERY known policy name from all previous
--     versions (old _staff / _admin / bootstrap / legacy
--     names like "members_delete_admin", "donations_staff",
--     etc.) before creating, so it never errors with
--     "policy ... already exists".
--   * Delete (মুছে ফেলা) এখন শুধু ADMIN role-এ সম্ভব।
-- ============================================================

-- ---------- 1. Role lookup (idempotent) ----------
...
-- ---------- 2. members ----------
-- Drop every policy name we have ever used on this table:
...
-- ============================================================
-- DONE — app থেকে sign out > sign in করুন একবার
-- ============================================================
